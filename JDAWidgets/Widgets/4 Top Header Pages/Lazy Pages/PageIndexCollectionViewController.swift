...
public protocol PageIndexCollectionViewControllerDataSource: AnyObject {
  ...
  /// Spacing between elements. Since horizontal scroll layoutt, minimum ` InteritemSpacing` is equal to `LineSpacing`
  var spacingForItem:CGFloat { get set }
}
...
private extension PageIndexCollectionViewController {
  ...
  func contentOffset(around index: CGFloat) -> CGPoint {
    ...
    var rightBorder = Double(collectionView.contentSize.width - collectionView.bounds.width)
